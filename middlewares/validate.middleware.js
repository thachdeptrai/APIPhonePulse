    ...
    const validationRules = Object.keys(schema).map(key => {
        ...
        Object.keys(rules).forEach(rule => {
            switch (rule) {
                ...
                case 'isEmail':
                    validator = validator.isEmail().withMessage(rules[rule].errorMessage || `${key} phải là email hợp lệ`);
                    break;
                
                case 'isLength':
                    validator = validator.isLength(rules[rule].options)
                        .withMessage(rules[rule].errorMessage || `${key} không đúng độ dài`);
                    break;
                
                case 'isNumeric':
                    validator = validator.isNumeric().withMessage(rules[rule].errorMessage || `${key} phải là số`);
                    break;
                
                case 'isBoolean':
                    validator = validator.isBoolean().withMessage(rules[rule].errorMessage || `${key} phải là boolean`);
                    break;
                
                case 'isURL':
                    validator = validator.isURL().withMessage(rules[rule].errorMessage || `${key} phải là URL hợp lệ`);
                    break;
                
                case 'isIn':
                    validator = validator.isIn(rules[rule].options)
                        .withMessage(rules[rule].errorMessage || `${key} không hợp lệ`);
                    break;
                
                case 'isDate':
                    validator = validator.isISO8601().withMessage(rules[rule].errorMessage || `${key} phải là ngày hợp lệ`);
                    break;
                
                case 'matches':
                    validator = validator.matches(rules[rule].pattern)
                        .withMessage(rules[rule].errorMessage || `${key} không đúng định dạng`);
                    break;
                
                case 'custom':
                    validator = validator.custom(rules[rule].validator)
                        .withMessage(rules[rule].errorMessage || `${key} không hợp lệ`);
                    break;
