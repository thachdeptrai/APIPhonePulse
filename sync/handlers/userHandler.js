...
module.exports = {
    ...
    try {
      ...
      if (!row.mongo_id) {
        ...
        if (existingDoc) {
          ...
          try {
            await conn.execute(`UPDATE users SET mongo_id = ? WHERE id = ?`, [
              existingDoc._id.toString(),
              row.id,
            ]);
            console.log(`🔗 Cập nhật mongo_id cho user id=${row.id}`);
          } finally {
            conn.release(); // 💯 Đảm bảo luôn giải phóng
          }
        }
      }
    } catch (err) {
  ...
  async syncDocumentToMySQL(doc) {
    ...
    try {
      ...
      console.error(`❌ Lỗi sync user ${doc._id}:`, err.message);
    } finally {
      connection.release(); // 💯 Giải phóng sạch
    }
  },
  ...
  async deleteFromMySQL(mongoId, connection) {
    try {
      ...
      if (result.affectedRows > 0) {
        ...
        console.warn(
          `⚠️ Không tìm thấy user với mongo_id=${mongoId} trong MySQL`
        );
      }
    } catch (err) {
  ...
  async deleteFromMongo(mongoId, mongooseModel) {
    ...
    if (rows.length > 0) {
      console.warn(
        `🚫 Bỏ qua xóa Mongo _id=${mongoId} vì user vẫn tồn tại trong MySQL`
      );
      return;
    }
